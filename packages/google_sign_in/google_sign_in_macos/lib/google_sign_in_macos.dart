...
/// MacOS implementation of [GoogleSignInPlatform].
class GoogleSignInMacOS extends GoogleSignInPlatform {
  /// This is only exposed for test purposes. It shouldn't be used by clients of
  /// the plugin as it may break or change at any time.
  @visibleForTesting
  MethodChannel channel =
      const MethodChannel('plugins.flutter.io/google_sign_in_macos');

  /// Registers this class as the default instance of [GoogleSignInPlatform].
  static void registerWith() {
    GoogleSignInPlatform.instance = GoogleSignInMacOS();
  }
  ...
  Future<void> init({
    ...
    if (signInOption == SignInOption.games) {
      throw PlatformException(
          ...
          message: 'Games sign in is not supported on MacOS');
    }
  ...
  Future<void> clearAuthCache({String? token}) async {
    // There's nothing to be done here on macOS since the expired/invalid
